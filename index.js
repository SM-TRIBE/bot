...
function handleDailyBonus(chatId) {
    ...
    if (lastDaily && (now - lastDaily) < 24 * 60 * 60 * 1000) {
        ...
        return bot.sendMessage(chatId, `You have already claimed your daily bonus. Please wait ${hoursLeft} more hours.`).catch(console.error);
    }
...
function handleSearchActions(query) {
    ...
    switch(subAction) {
        ...
        case 'criteria':
            ...
            else if (field === 'interests') executeSearch(message.chat.id, message.message_id);
            break;
        case 'result':
...
async function promptSearchCriteria(chatId, criteria, messageId) {
    ...
    if (criteria === 'interests' && userState[chatId]?.search?.interests === 'yes') {
        userState[chatId].search.step = 'interests_input';
        return bot.editMessageText("Please type the interests you're looking for, separated by commas.", { chat_id: chatId, message_id: messageId });
    }
...
function executeSearch(chatId, messageId) {
    ...
    const results = Object.values(db.users).filter(u => {
        if (u.id === chatId || u.banned) return false;
